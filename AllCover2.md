...
## I/O搬运的方式
> File文件夹装，流运输（inputStream,outputStream，fileinputStream这些就叫装饰者，也就是提供格外服务），UTF-8中文字符其实是占3字节
，数据运输其实基本上以流的形式（读文件也是读字节流，它可以把字节流转换为字符流，然后我们把得到用new String就可以用了，很方便；outputStreamWriter，是把我们写的str字符包装成字节流，输出），序列化看起来是像是把类的实例对象变成一种写的时候能在流中流淌，但读的时候又能直接读出对象而不是零零散散的字节的操作,transient短暂的不序列化的（ArrayList的数组是transient的动态扩展，内容才是Serializable）

> NIO,以块的形式传递数据，块是多个字节；流是单向传输的，通道是双向的；通道一般数据先存在缓冲区，读入数据到缓冲区后如果要写基本上先flip()，它会把position归0让你从通道的开头开始写，写多长flip把归0前的position给了limit，写limit长度。

> 套接字通道的不是文件通道的，套接字通道有selector事件触发也就是非阻塞！

## BIO,NIO,AIO
> 同步与阻塞其实是两个概念，你会看到有同步阻塞和同步非阻塞的情况。同步异步着重在调用有没有返回一个结果，异步返回的是对方知道了，但对方可以用回调机制做好了再叫你；阻塞非阻塞着重的是线程有没有被挂起，能不能去做别的事情，异步是可以做但他只关注有没“调用结果”。同步可以非阻塞，就是没做好还没返回结果，我也可以去做别的事，只是需要时不时来检查，异步就是他做好了会通知你，你不用时时刻刻检查。

> selector使得不用一个socket链接就创建一个线程
