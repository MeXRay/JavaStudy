...
>  线程：一个进程有main主线程和其他线程构成，线程共享进程的堆和方法区，却又有独自的程序计数器（指向指定地址），虚拟机栈（java方法的局部变量）和本地方法栈（本地方法的局部变量，本地方法的意思却是调用C语言，可能本地指在不懂地调用非java语言吧）。
  ...
  * 进程和线程的关系，区别和优缺点
    ...
    一个进程有多个线程，线程是进程的更小执行单元。区别是原来线程会相互影响，因为共用了堆的对象和方法区的一些变量；进程是独立，进程会可能会共享同一个外部资源，但不会分享自己的内部资源，所以不会互相影响。因此优缺点很明显：进程的资源安全问题更好，适合管理隔离一些资源；而线程有切换速度快，开销小的特点。
...
# java并发进阶
  * synchronized的了解，使用，底层，JDK1.6 之后的底层优化，谈谈 synchronized和ReentrantLock 的区别
   ...
   * JMM是什么， 说说 synchronized 关键字和 volatile 关键字的区别
     ...
     它的访问规范是这样的：主内存存放所有变量，线程有各自的独立工作内存，里面存放的主内存的部分拷贝变量！问题在于多线程访问，可能一个线程改变了某一变量，但另一线程还在用没更新的拷贝副本变量操作。所以要改变之后立马同步到主内存，用这个变量就去主内存更新变量值，所以volatile就是说明这个变量是同步的，是需要这么做的。
...
# 分布式
  ...
  * 分布式事务要做到什么，2PC，存在的问题（想象图）
     ...
     分布式事务提交还分两个阶段啊，用的是协调者（Coordinator，只有一个崩了就没了）,询问参与者（线程事务）执行完没（这个过程阻塞），协调者只有在
     所有都执行成功情况下才提交（一个错就算全部错，都不给降级机会）
...
# 消息队列
  ...
  * 消息模型有哪两个？和观察者模式相比有什么不同
     ...
     消息模式就是消息的传递接收方式，所以有点对点和发布/订阅（感觉区别就是消息队列里的消息是允许一个消费者还是多个消费者）
     
     观察者模式也是发布/订阅，只是他没有消息队列，所以它的主题和观察者是直接交流的，同时它是同步阻塞的。
  * 使用场景，就是问好处价值市场
    
    有了消息队列这个中间件，就是有了把一时处理不了的事情放在这，最后一定会做，这是异步，是缓冲，所以可以用在削峰、流控，分布式事务执行；同时它是解耦的，所以是易扩展的。
  * 发送端和接收端的可靠性体现，实现方式
     
     发送端的可靠性指的是一定发出去/接收到，它的实现就是发送端，只有把消息存入消息队列才会停止转发（在数据库建消息表，转发成功才删除数据）；接收端的可靠性原来指的是一次消费，不重复消费，所以用了幂等性/或记录已消费的消息表
# RPC
  * 概念，主要技术，实现过程，方案，“使得开发分布式程像本地程序一样简单”？
    
     远端过程控制，是为了在远端能调用本地的服务（方法），像在同一台机子开发一样。
     
     主要技术是估计技术难点是数据传送，所以有序列化，反序列化和数据传输协议。
     
     方案只记得我的dubbo，没想到hessian不属于dubbo，还有JDK自带的RMI(Remote Method Invocation）,还有个貌似全能的Thrift
  * RPC四种框架，选择的依据：代码入侵，需要长连接，需要跨越网段和防火墙
     代码入侵是代码得用代码生成器生成？（Thrift）长连接有：dubbo,Thrift（他们基于TCP） 网段、防火墙：基于HTTP的hesian和Thrift的HTTP Transport
  * Dubbo 分布式服务不等于微服务 架构图中服务容器是负责启动，加载，运行提供者的 注册中心有数据库的缓存，消费者有提供者列表缓存（意味着可以在无注册中心情况下直连） 服务端无状态 我的淘淘商城用的就是dubbo直连（消费者直接指定服务者，绕过了注册中心的查询）
  ...
  * 消息队列 削峰和流控  常见消息队列在吞吐量、可用性、时效性、功能支持上的比较
     ...
     主从和分布式不一样，主从是集群，集群是很多台计算机做自己的事（可以是相同事情但不同服务）；分布式是把同一个服务拆分，多台一起做。所以RocketMQ和kafka更讲究分治合作。
     
     RabbitMQ基于erlang语言，并发性好，延时在微秒级，其他在毫秒级（语言这么重要）
     
     kafka更多用于日志记录
  * RabbitMQ在提供者和消息队列中间还有交换器（路由选择哪个队列） 把broker中断器当做服务器、服务节点好奇怪啊
  * RabbitMQ交换器4种类型
  
     所有消息都给绑定的队列(，routekey匹配bindingkey(direct)，用模糊查询去分配给符合表达式的队列(topic)，headers匹配  
